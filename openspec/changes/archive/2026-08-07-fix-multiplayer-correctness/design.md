...
### D3: 重连路径 — 场景 A 不重建世界,复用 driver Network 管线

客户端接通 `apply_reconnect`(network.rs:280)。重连恢复区分两个场景:

- **场景 A(网络断开,进程存活)**:本地世界在断点处完好,**不重建**。收到 `ReconnectResponse` 后:
  1. `reconnect_recovery_system`(transport.rs)消费 `NetworkEvent::Reconnect`,并同步处理 `GameJoined` 更新 player_id
  2. `apply_reconnect` 校验 `ruleset_version` 并把断点后 `ticks` 灌入 `relay_buffer`
  3. `simulation_driver_system` 依 `is_tick_ready` 消费;断点期间 accumulator 持续积累,恢复后单帧追平到 relay 当前 tick,再正常 lockstep(`reconnect_catchup` 集成测试验证)
- **场景 B(进程重启)**:需重建世界,走 `rebuild_world`(D5)。本变更未接线,spec 已明确为后续 change。

**R1 强制**:场景 B 重建**禁止** `init_simulation_world`(2 槽)/`run_tick_default`(AI 开)——必须 `init_simulation_world_multi` + `run_tick(enable_ai:false)`,与正常网络路径一致(driver.rs:347-355)。现有 network.rs 注释已修正。

**Alternatives**: 独立 while 循环快进 —— 绕过 driver 管线,易引入与正常路径的 AI/NoOp 差异。复用管线保证同一仿真入口(宪法 §9.1)。

### D4: 地图一致性 — 场景 A 不重建,地图保持对局

重连场景 A 不重建世界,地图保持当前对局地图(render_view 网络启动用 Medium,对局中 current_map_size 即 Medium),天然一致。`map_spec_hash` 当前全仓硬编码 0(relay_core/thread/relay 未计算),故**未实现** hash→MapSize 映射;场景 B 重建时由调用方以对局 map_size 调 generate_map,完整映射留待场景 B 接入。

**Alternatives**: 协议新增 MapSize 字段 —— 协议面更宽;当前场景 A 无需映射。
...
- 场景 B(进程重启)接入:`ReconnectRequest(last_tick_consumed=0)` 拉全量日志 + `rebuild_world` + 本地 replay 重放断点前 —— 后续 change
- `map_spec_hash` 真正计算 + →MapSize 映射 —— 后续 change(当前恒 0,场景 A 不需要)
