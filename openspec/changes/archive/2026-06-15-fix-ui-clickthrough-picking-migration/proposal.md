...
- 用 `Interaction::Pressed` 检测替代 `UiFocusBlocker`，在 `selection_click_system` 和 `command_issue_system` 中检查是否有 UI 元素正在被按下
- 删除 `UiFocusBlocker` 资源、`reset_ui_focus_blocker` 系统及其所有引用
- 透明 UI 容器添加 `Pickable::IGNORE`，防止阻挡游戏世界的 picking（预防性措施）
...
- `ui-click-detection`: 用 `Interaction::Pressed` 检测替代手动 UiFocusBlocker，实现 UI 点击穿透保护
...
- `crates/render_view/src/selection.rs`: `selection_click_system` 和 `command_issue_system` 新增 `Interaction::Pressed` 检查
- `crates/render_view/src/ui/hud.rs`: 删除 `UiFocusBlocker` 定义、`reset_ui_focus_blocker` 系统、3 个 seek panel 系统中的 blocker 参数；透明容器添加 `Pickable::IGNORE`
...
- 无新增依赖（`Interaction` 来自 bevy_ui，已随 bevy 默认包含）
