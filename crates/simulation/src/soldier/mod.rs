...
pub struct Movement {
    ...
    pub command_target: Option<UnitId>,
    pub waypoint: Option<FixedVec2>,
    pub force_move: bool,
}
#[derive(Component, Clone, Debug)]
pub struct SeekStance {
    pub active: bool,
    pub seek_range: u32,
}
#[derive(Component, Clone, Debug)]
...
#[derive(Component, Clone, Debug)]
pub struct CityOrigin(pub UnitId);
#[derive(Component, Clone, Debug)]
...
#[derive(Component, Clone, Debug)]
pub struct SoldierStateComponent(pub SoldierState);
#[derive(Component, Clone, Debug)]
...
#[derive(Component, Clone, Debug)]
pub struct CityRadius(pub u32);
#[derive(Component, Clone, Debug)]
pub struct AuraHealComponent {
    pub base_heal: u32,
    pub per_level_heal: u32,
}
#[derive(Component, Clone, Debug)]
pub struct SpawnDirection(pub FixedVec2);
        ...
        match cmd.action {
            ...
            Action::Attack { unit, target } => {
                if let Some(e) = find_entity_by_unit_id(world, unit) {
                    ...
                    world.entity_mut(e).insert(Movement {
                        speed: s,
                        target: Some(target),
...
pub fn soldier_movement_system(world: &mut World) {
...

    {
        ...
        for (e, pos, mov, st, sst, slow, shield, facing_dir) in q.iter(world) {
            ...
            let new_pos =
                FixedVec2::new(Fixed(pos.0.x.0 + dx as i64), Fixed(pos.0.y.0 + dy as i64));
...
pub fn city_spawn_system(world: &mut World) {
    ...
    for (entity, pos, faction, spawn_type, _cooldown, can_spawn) in cities {
        if can_spawn {
            ...
            let spawn_pos = FixedVec2::new(
                pos.x + Fixed::from_int(30 + jx),
                pos.y + Fixed::from_int(jy),
            );
            ...
            let mut seek_stance = SeekStance {
                active: false,
                seek_range: 0,
            };
            ...
            for d in &seek_directives.0 {
                ...
                if matches && d.issue_tick >= best_tick {
                    seek_stance = SeekStance {
                        active: d.seek_range > 0,
                        seek_range: d.seek_range,
                    };
            ...
            let soldier_entity = world
                .spawn((
                    UnitIdComponent(new_id),
                    SoldierMarker,
                    ...
                    Attack {
                        ...
                        cooldown_remaining: cfg.attack_interval_ticks,
                    },
                    FactionComponent(faction),
                    SoldierTypeComponent(spawn_type),
                    Level { level: 1, exp: 0 },
                    CityOrigin(origin),
            ...
            if spawn_type == SoldierType::Infantry {
                ...
                world.entity_mut(soldier_entity).insert(ShieldItem {
                    hp: combat_config.shield.initial_hp,
                    max_hp: combat_config.shield.initial_hp,
                });
            }
            world
                .entity_mut(soldier_entity)
                .insert(crate::types::AttackWindup {
                    remaining_ticks: 0,
                    target: None,
                });
            ...
            events.spawned.push(UnitSpawned {
                ...
                pos: spawn_pos,
                faction,
                unit_kind: UnitKind::Soldier(spawn_type),
            });
        } else {
            // Decrement cooldown
...
pub fn city_capture_check_system(world: &mut World) {
    ...
    for (entity, city_id, old_faction, new_faction) in captures {
        ...
        let mut events = world.resource_mut::<SimulationEvents>();
        events.captured.push(CityCaptured {
            city_id,
            old_faction,
            new_faction,
        });
    ...
    for si in soldiers {
        for ci in &cities {
            ...
            if si.faction != ci.faction {
                ...
                if let Some(o) = world.entity(si.entity).get::<CityOrigin>() {
                    origin_decrements.push(o.0);
                }
...
pub fn aura_heal_system(world: &mut World) {
    ...
    let cities: Vec<(FixedVec2, Faction, u32, u32)> = {
        ...
        q.iter(world)
            .map(|(p, f, c, r)| (p.0, f.0, c.level, r.0))
            .collect()
    };
        ...
        for (e, sp, sf, _) in q.iter(world) {
            ...
            for (cp, cf, cl, cr) in &cities {
                ...
                if (sp.0 - *cp).length_squared() <= eff * eff {
                    total += city_config.aura.base_heal
                        + (cl.saturating_sub(1)) * city_config.aura.per_level_heal;
                }
            }
            if total > 0 {
                heals.push((e, total));
...
pub fn shield_pickup_system(world: &mut World) {
    ...
    for sd in &soldiers {
        ...
        for &(dropped_e, dropped_pos, dropped_hp, dropped_max_hp) in &dropped {
            if claimed_dropped.contains(&dropped_e) {
                continue;
            }
    ...
    for (soldier_e, dropped_e, hp, max_hp) in pickups {
        world
            .entity_mut(soldier_e)
            .insert(ShieldItem { hp, max_hp });
...
pub fn slow_debuff_tick_system(world: &mut World) {
    let expired: Vec<Entity> = {
        ...
        q.iter(world)
            .filter(|(_, s)| s.remaining_ticks <= 1)
            .map(|(e, _)| e)
            .collect()
    };
    let mut q = world.query::<(Entity, &mut SlowDebuff)>();
    for (_, mut s) in q.iter_mut(world) {
        s.remaining_ticks = s.remaining_ticks.saturating_sub(1);
    }
    for e in expired {
        world.entity_mut(e).remove::<SlowDebuff>();
    }
}
...
pub fn fearless_buff_tick_system(world: &mut World) {
    let expired: Vec<Entity> = {
        ...
        q.iter(world)
            .filter(|(_, f)| f.remaining_ticks <= 1)
            .map(|(e, _)| e)
            .collect()
    };
    let mut q = world.query::<(Entity, &mut FearlessBuff)>();
    for (_, mut f) in q.iter_mut(world) {
        f.remaining_ticks = f.remaining_ticks.saturating_sub(1);
    }
    for e in expired {
        world.entity_mut(e).remove::<FearlessBuff>();
    }
}
...
pub fn soldier_level_up_system(world: &mut World) {
    ...
    let mut leveled: Vec<(Entity, UnitId, u32)> = Vec::new();
    {
        ...
        for (e, id, mut lvl) in q.iter_mut(world) {
            ...
            while lvl.exp >= lu.exp_to_level {
                lvl.exp -= lu.exp_to_level;
                lvl.level += 1;
                changed = true;
            }
            if changed {
                leveled.push((e, id.0, lvl.level));
            }
        }
    }
    for (e, uid, nl) in leveled {
        ...
        if let Some(mut atk) = em.get_mut::<Attack>() {
            atk.damage += lu.attack_gain;
        }
        let mut events = world.resource_mut::<SimulationEvents>();
        events.leveled_up.push(SoldierLeveledUp {
            unit_id: uid,
            new_level: nl,
        });
    }
...
mod seek_stance_tests {
    ...
    fn test_spawn_inherits_global_seek_all() {
        ...
        world
            .resource_mut::<GlobalSeekDirective>()
            .0
            .push(SeekDirective {
                scope: SeekScope::All,
                seek_range: 10,
        ...
        assert!(
            found,
            "Newly spawned soldier should inherit All seek directive"
        );
    }
    ...
    fn test_spawn_ignores_non_matching_bytype() {
        ...
        assert!(
            !militia_with_seek,
            "Militia should not inherit Archer-only directive"
        );
    }
    ...
    fn test_spawn_latest_matching_directive_wins() {
        ...
        // Push All(10) at tick 5, then ByType(Militia, 30) at tick 8
        {
            ...
            gd.0.push(SeekDirective {
                scope: SeekScope::All,
                seek_range: 10,
                issue_tick: 5,
            });
            gd.0.push(SeekDirective {
                scope: SeekScope::ByType(SoldierType::Militia),
                seek_range: 30,
                issue_tick: 8,
            });
        }
        ...
        assert!(
            found,
            "Militia should use latest matching directive (tick 8, range 30)"
        );
    }
    ...
    fn test_consume_seek_stance_global_all() {
        let mut world = init_simulation_world(42);
        let p1 = spawn_player_soldier(
            &mut world,
            FixedVec2::new(Fixed::from_int(0), Fixed::from_int(0)),
            SoldierType::Militia,
        );
        let p2 = spawn_player_soldier(
            &mut world,
            FixedVec2::new(Fixed::from_int(50), Fixed::from_int(0)),
            SoldierType::Archer,
        );
        spawn_enemy_soldier(
            &mut world,
            FixedVec2::new(Fixed::from_int(100), Fixed::from_int(0)),
        );
        ...
        assert!(
            s1.active && s1.seek_range == 15,
            "Militia should have active seek range 15"
        );

        let e2 = find_entity_by_unit_id(&mut world, p2).unwrap();
        let s2 = world.entity(e2).get::<SeekStance>().unwrap();
        assert!(
            s2.active && s2.seek_range == 15,
            "Archer should have active seek range 15"
        );
    }
    ...
    fn test_consume_seek_stance_by_type() {
        let mut world = init_simulation_world(42);
        let p_inf = spawn_player_soldier(
            &mut world,
            FixedVec2::new(Fixed::from_int(0), Fixed::from_int(0)),
            SoldierType::Infantry,
        );
        let p_arch = spawn_player_soldier(
            &mut world,
            FixedVec2::new(Fixed::from_int(50), Fixed::from_int(0)),
            SoldierType::Archer,
        );
        ...
        world.resource_mut::<CommandBuffer>().push(GameCommand {
            ...
            player_id: 0,
            action: Action::SetSeekStance {
                scope: SeekScope::ByType(SoldierType::Infantry),
                seek_range: 20,
                unit_ids: vec![],
            },
        });
        ...
        assert!(
            s_inf.active && s_inf.seek_range == 20,
            "Infantry should have active seek range 20"
        );
        ...
        assert!(
            !s_arch.active && s_arch.seek_range == 0,
            "Archer should remain default (not Infantry type)"
        );
    }
    ...
    fn test_consume_seek_stance_selection() {
        ...
        let s1 = world.entity(e1).get::<SeekStance>().unwrap();
        assert!(
            s1.active && s1.seek_range == 60,
            "Selected cavalry should have active seek range 60"
        );
        ...
        let s2 = world.entity(e2).get::<SeekStance>().unwrap();
        assert!(
            !s2.active && s2.seek_range == 0,
            "Unselected cavalry should remain default"
        );
        ...
        assert!(
            gd.0.is_empty(),
            "Selection command should not modify GlobalSeekDirective"
        );
    }
    ...
    fn test_consume_seek_stance_range_zero_disables() {
        ...
        let s1 = world.entity(e1).get::<SeekStance>().unwrap();
        assert!(
            !s1.active && s1.seek_range == 0,
            "range=0 should disable seek"
        );
    }
    ...
    fn test_engagement_inactive_seek_no_target() {
        ...
        let e1 = find_entity_by_unit_id(&mut world, p1).unwrap();
        world.entity_mut(e1).insert(SeekStance {
            active: false,
            seek_range: 0,
        });
    ...
    fn test_engagement_active_seek_in_range() {
        ...
        let enemy_uid = spawn_enemy_soldier(
            &mut world,
            FixedVec2::new(Fixed::from_int(100), Fixed::from_int(0)),
        );
    ...
    fn test_engagement_active_seek_out_of_range() {
        ...
        spawn_enemy_soldier(
            &mut world,
            FixedVec2::new(Fixed::from_int(200), Fixed::from_int(0)),
        );
        ...
        assert!(
            mov.target.is_none(),
            "Out-of-range enemy should not be targeted"
        );
    }
    ...
    fn test_engagement_force_move_skips_seek() {
        ...
        world.entity_mut(e1).insert(Movement {
            speed: 80,
            target: None,
            command_target: None,
            waypoint: Some(FixedVec2::new(Fixed::from_int(500), Fixed::from_int(0))),
            force_move: true,
        });
        ...
        assert!(
            mov.target.is_none(),
            "force_move should skip auto-engagement"
        );
    }
...
mod shield_lifecycle_tests {
    ...
    fn spawn_infantry(world: &mut World, pos: FixedVec2, faction: Faction) -> UnitId {
        ...
        world
            .entity_mut(e)
            .insert(crate::types::FacingDirection { angle: Fixed::ZERO });
    ...
    fn test_infantry_spawns_with_shield() {
        ...
        assert!(
            world.get::<ShieldItem>(e).is_some(),
            "Infantry should have ShieldItem"
        );
        assert!(
            world.get::<ShieldComponent>(e).is_some(),
            "Infantry should have ShieldComponent"
        );
    ...
    fn test_militia_spawns_without_shield() {
        ...
        let e = find_entity_by_unit_id(&mut world, uid).unwrap();
        assert!(
            world.get::<ShieldItem>(e).is_none(),
            "Militia should NOT have ShieldItem"
        );
        assert!(
            world.get::<ShieldComponent>(e).is_none(),
            "Militia should NOT have ShieldComponent"
        );
    }
    ...
    fn test_city_spawn_infantry_has_shield_militia_does_not() {
        ...
        let mut found_infantry_with_shield = false;
        {
            ...
            for (fac, st, shield_item, shield_comp) in q.iter(&world) {
                if fac.0 == Faction::Player && st.0 == SoldierType::Infantry {
                    ...
                    assert!(
                        shield_comp.is_some(),
                        "Spawned Infantry should have ShieldComponent"
                    );
        ...
        assert!(
            found_infantry_with_shield,
            "Should have spawned at least one Infantry"
        );
    }
    ...
    fn test_shield_pickup_no_existing_shield() {
        ...
        assert!(
            world.get::<ShieldItem>(e).is_some(),
            "Militia should have picked up ShieldItem"
        );
        assert!(
            world.get::<ShieldComponent>(e).is_some(),
            "Militia should have ShieldComponent"
        );
        let shield = world.get::<ShieldItem>(e).unwrap();
        ...
        assert_eq!(
            q.iter(&world).count(),
            0,
            "DroppedShield should be consumed"
        );
    }
    ...
    fn test_shield_pickup_ignores_worse_shield() {
        ...
        assert_eq!(
            q.iter(&world).count(),
            1,
            "DroppedShield should remain (not picked up)"
        );
    }
    ...
    fn test_shield_pickup_archer_ignored() {
        ...
        world.spawn((
            UnitIdComponent(uid),
            SoldierMarker,
            LogicalPosition(FixedVec2::new(Fixed::from_int(10), Fixed::from_int(10))),
            Movement {
                speed: cfg.speed,
                target: None,
                command_target: None,
        ...
        assert!(
            world.get::<ShieldItem>(e).is_none(),
            "Archer should not pick up shields"
        );
    }
    ...
    fn test_shield_decay_not_yet_expired() {
        ...
        assert_eq!(
            q.iter(&world).count(),
            1,
            "Shield should not have decayed yet"
        );
    }
    ...
    fn test_shield_decay_multiple_mixed_ages() {
        ...
        assert_eq!(
            remaining.len(),
            1,
            "Only the expired shield should be removed"
        );
        assert_eq!(
            remaining[0].1.drop_tick, 500,
            "The remaining shield should be the newer one"
        );
    }
    ...
    fn test_aura_heal_heals_shield() {
        ...
        assert!(
            shield.hp > 1000,
            "Shield HP should have been healed (was 1000, now {})",
            shield.hp
        );
        assert!(
            shield.hp <= shield.max_hp,
            "Shield HP should not exceed max"
        );
    }
    ...
    fn test_full_shield_lifecycle() {
        ...
        assert!(
            ...
            "Militia should have picked up the dropped shield"
        );
