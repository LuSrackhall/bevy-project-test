...
impl Plugin for UiPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<hud::HudTexts>()
            ...
            // LAN Lobby
            .add_systems(
                OnEnter(crate::GameState::LanLobby),
                lan_lobby::setup_lan_lobby,
            )
            .add_systems(
                OnEnter(crate::GameState::LanLobby),
                lan::start_lan_discovery,
            )
            .add_systems(
                OnExit(crate::GameState::LanLobby),
                lan_lobby::cleanup_lan_lobby,
            )
            .add_systems(OnExit(crate::GameState::LanLobby), lan::stop_lan_discovery)
            .add_systems(
                Update,
                lan::update_lan_servers.run_if(in_state(crate::GameState::LanLobby)),
            )
            // LanLobby room list and modal
            .add_systems(
                Update,
                lan_lobby::update_room_list.run_if(in_state(crate::GameState::LanLobby)),
            )
            ...
            .add_systems(
                OnEnter(crate::GameState::Lobby),
                |mut rs: ResMut<lobby::ReadyState>| rs.0 = false,
            )
            .add_systems(
                OnExit(crate::GameState::Lobby),
                (lobby::cleanup_lobby_ui, crate::cleanup_lobby_network),
            )
            .add_systems(
                Update,
                lobby::update_lobby_status
                    .after(crate::lobby_update_system)
                    .run_if(in_state(crate::GameState::Lobby)),
            )
            .add_systems(
                Update,
                lobby::update_lobby_player_list
                    .after(crate::lobby_update_system)
                    .run_if(in_state(crate::GameState::Lobby)),
            )
            .add_systems(
                Update,
                lobby::update_ready_button
                    .after(lobby::update_lobby_status)
                    .run_if(in_state(crate::GameState::Lobby)),
            )
            .add_systems(
                Update,
                lobby::toggle_button_visibility.run_if(in_state(crate::GameState::Lobby)),
            )
