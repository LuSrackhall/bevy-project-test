...
- **民兵**：无特殊效果
- **步兵**：
  - **举盾状态切换**：框选步兵后可切换举盾/正常状态
    - 举盾时移速降低 30 点（作用于减速效果生效前，即移速变为 50 px/s）
    - 举盾时获得正面免伤：免疫 80% 来自正面的所有伤害
    - 举盾时被动格挡：面对远程攻击时自动格挡，将伤害降低至 20%
    - 批量框选切换规则：若选中步兵中有举盾和正常两种状态，统一全部变为举盾（即用户想全部正常最多需点两次切换，想全部举盾最多一次）
- **弓兵**：
...
所有士兵具有面朝方向属性，影响正面/背面判定：
- 士兵移动时面朝移动方向
- 士兵攻击时面朝攻击目标
- 面朝方向决定正面免伤区域（步兵举盾时生效）
- 背面受到攻击时无免伤效果

### 攻击蓄力机制

士兵在移动中发起攻击时需要先停下完成蓄力：
- 攻击前摇：士兵从移动状态切换到攻击状态时，需要先停下完成蓄力动作
- 蓄力期间无法移动，蓄力完成后才能发起攻击
- 站桩攻击时无蓄力惩罚，可直接按攻击间隔发起攻击
...
基础伤害 = 攻击者（攻击力 + 等级加成 + 无畏加成）
  × 其余组合: 1.0
  [骑兵目标] → roll 闪避 → 成功则伤害 = 0（触发无畏）（闪避优先于被动格挡）
  [举盾步兵正面] → ×0.2（80% 免伤）
...
士兵进入攻击范围 → SoldierState = Fighting
  → 面朝方向更新为朝向攻击目标
  → 若从移动状态切换：先停下完成攻击蓄力，蓄力期间无法移动
  → 每 0.5s 攻击间隔判定：
      [骑兵目标] → roll 闪避 → 成功 → 无伤害 + 触发无畏（持续 2s）（闪避优先于被动格挡）
      [举盾步兵正面] → 伤害 × 0.2（80% 免伤）
      [弓兵] → 随机触发多重射击（2~5 目标，机率 = clamp(5% + 等级/2, 5%, 30%)）
             → 生成 1~N 个 Arrow Entity（固定方向飞行，400 单位/秒）
             → Arrow 命中目标时：
                 [举盾步兵正面] → 被动格挡，伤害降至 20%
                 [其余] → 结算伤害 + 减速 debuff
      [其余目标] → 计算伤害 → 扣除血量
...
```
框选步兵 → 底部工具栏显示 [🛡️举盾/正常] 按钮
批量规则：若选中步兵中有举盾和正常混存 → 统一切为举盾（先强制统一）
         → 再次点击切换 → 全部变正常（最多两次操作完成统一切换）
举盾效果：
  - 移速降低 30 点（在减速效果生效前，即举盾步兵基础移速 = 50 px/s）
  - 正面 80% 免伤（所有伤害类型）
  - 被动格挡：面对远程攻击时自动格挡，将伤害降低至 20%
```
