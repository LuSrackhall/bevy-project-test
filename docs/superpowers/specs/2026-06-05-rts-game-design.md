...
士兵进入攻击范围 → SoldierState = Fighting
  → 每 0.5s 攻击间隔判定：
      [骑兵目标] → roll 闪避 → 成功 → 无伤害 + 触发无畏（持续 2s）
      [举盾步兵正面] → 伤害 × 0.2（80% 免伤）
      [弓兵] → 随机触发多重射击（2~5 目标，机率 = clamp(5% + 等级/2, 5%, 30%)）
             → 生成 1~N 个 Arrow Entity（追踪目标，400px/s）
             → Arrow 到达目标时：
                 [举盾步兵正面 + 70% 几率] → 拦截，盾兵承伤（×0.2）
                 [其余] → 结算伤害 + 减速 debuff
      [其余目标] → 计算伤害 → 扣除血量
...
举盾效果：
  ...
  - 正面 80% 免伤（所有伤害类型）
  - 70% 几率拦截经过正面的飞行道具（Arrow），拦截后盾兵承担该伤害（受 80% 免伤）
```
